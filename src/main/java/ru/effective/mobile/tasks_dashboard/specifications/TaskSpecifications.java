...
public class TaskSpecifications {
    ...
    public static Specification<Task> hasAuthorName(String authorName) {
        return (root, query, criteriaBuilder) -> {
            return criteriaBuilder.like(criteriaBuilder.lower(root.get("author").get("name")), "%" + authorName.toLowerCase() + "%");
        };
    }

    public static Specification<Task> hasExecutorName(String executorName) {
        return (root, query, criteriaBuilder) -> {
            return criteriaBuilder.like(criteriaBuilder.lower(root.get("executor").get("name")), "%" + executorName.toLowerCase() + "%");
        };
    }
}
